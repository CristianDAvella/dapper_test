"""
Módulo de extracción de datos desde el sitio web de ANI.
Mantiene intacta la lógica de scraping existente.
"""
...
def clean_quotes(text):
    """Elimina comillas y caracteres especiales del texto."""
    if not text:
        return text
...
# ==================== EXTRACCIÓN DE CAMPOS DE UNA FILA ====================
...
def run_extraction(num_pages=9, verbose=False):
    """
    Función principal de extracción para ser llamada desde Airflow.
    
    Args:
        num_pages (int): Número de páginas a scrapear (desde 0 hasta num_pages-1)
        verbose (bool): Si mostrar logs detallados
    
    Returns:
        pd.DataFrame: DataFrame con todos los registros extraídos
    """
    print(f"=== INICIANDO EXTRACCIÓN: {num_pages} páginas ===")
    ...
    print(f"=== EXTRACCIÓN COMPLETADA: {len(df_normas)} registros totales ===")
