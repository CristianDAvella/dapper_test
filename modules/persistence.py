"""
Módulo de persistencia de datos en PostgreSQL.
Maneja la conexión a la base de datos y la inserción evitando duplicados.
"""
...
from .constants import ENTITY_VALUE
...
def insert_new_records(db_manager, df, entity):
    ...
    try:
        ...
        # 10. MENSAJE FINAL CON ESTADÍSTICAS
...
def run_persistence(df):
    """
    Función principal de persistencia para ser llamada desde Airflow.
    
    Args:
        df (pd.DataFrame): DataFrame validado con los registros a insertar
    
    Returns:
        dict: Diccionario con resultados de la inserción
    """
    print(f"=== INICIANDO PERSISTENCIA: {len(df)} registros ===")
    ...
    try:
        inserted_count, status_message = insert_new_records(db_manager, df, ENTITY_VALUE)
